...
- **`lineups.proj_rank` and `geo_rank` changed INTEGER → REAL; `SCHEMA_VERSION` 1 → 2.** All three rank columns are average-ranks that split ties; the pinned DDL only marked `own_rank` REAL because the sample slate happened to show ties only there. Across the 43 reconciled slates `Proj_Rank` is fractional 2266× and `Geo_Rank` 3002×. `db/schema.py:migrate()` handles v1→v2 by recreating the (necessarily empty) `lineups` table, and refuses rather than dropping data if it isn't empty. It detects drift from the **live column types**, not `user_version` — `init_db`'s DDL is all `IF NOT EXISTS`, so it can stamp a version onto a schema it never changed. `init_db` calls `migrate` itself, so every entry point is safe.
