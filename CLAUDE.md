...
- **CI runs ruff + pytest on every push to `main` and every PR** (`.github/workflows/ci.yml`, added 2026-09-07). Ubuntu, matrixed over Python **3.13 and 3.14** — that matrix is the only thing testing the cloud-fallback claim above, that the lockfile pins identical package versions on both interpreters. `uv sync --locked` means a lockfile that has drifted from `pyproject.toml` is itself a failure. **It exists because no reviewer could ever run the suite:** across three review rounds on PR #11, neither the Claude reviewer nor CodeRabbit could execute `pytest` or `ruff` in their sandbox, so every "tests pass" claim rested on the author alone. Keep this workflow separate from the `claude-auto-pr-*` ones — those are agents that read and comment, this is a build.
