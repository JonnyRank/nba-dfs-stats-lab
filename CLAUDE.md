...
- **Phase 4 dry-run gate cleared** (2026-07-28, `uv run python scripts/verify_phase4.py` all PASS, then `uv run python -m nba_dfs_stats_lab.ingest.orchestrator --dry-run` over all 412 slates):
  ...
  - **Exactly two warnings in the whole run**, both on `2026-02-02_classic_main`: `Team`/`Opponent` 36 missing values each. That is the same 36 nullable rows the original 409-file probe found, now pinned to one file. No `Actual_FPTs` warning anywhere — every salary file has actuals on every row.
