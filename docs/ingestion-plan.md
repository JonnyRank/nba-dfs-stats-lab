...
- `db/schema.py`: a new `fpts_audit` table at the `(slate_id, dk_id)` grain — a **full census**, one row per `slate_players` row, carrying the pristine `dk_value`, the `ops_value`, the delta, an `action`, a `reason` and an `off_slate` flag. `SCHEMA_VERSION` 2 → 3, with a `migrate()` step that **refuses** rather than dropping a populated audit: once corrections are applied, `dk_value` is the only surviving record of what DraftKings said.
...
- **Three findings that shape the design** — all three are in CLAUDE.md Decisions:
  - **The join must resolve a *game date*, not assume the slate date.** DK ran genuine two-calendar-day slates in the conference finals: `Main-2026-05-17.csv` carries both CLE/DET and SAS/OKC, but ops dates SAS@OKC to 05-18. 214 non-zero rows sit on the far side and every one is rescued by ops date +1 with the value matching exactly. **As built: 10 such slate dates, 20 game-sides** (`2026-05-12`, `05-17`, and every slate `05-18`→`05-25`) — two series on alternating nights, both listed on every file. Also: +1 must be tried before −1, since only a game yet to tip off can be on a slate.
...
- ✋ **Gate — CLEARED 2026-09-07** (`uv run python scripts/verify_phase6.py`, then `--write`): **all 27 checks PASS.** 51,971 `fpts_audit` rows written, 930 `actual_fpts` values corrected, ops row counts identical before and after. Run in two stages like Phase 5: **report-only first**, so the change list — and a *What --write will do* preview — is in front of Jonny before anything writes, then `--write` and re-check against the DB. It shipped with 27 checks rather than the 14 sketched here: the same questions, split so a failure names one thing, plus the write-stage and idempotency checks they imply. The *what would make this FAIL* table is in the spec doc §3.6, and §5.3 lists the as-built set. The headline ones:
  ...
  - ~~Expected on today's data: ~1,144 corrected, ~30,800 unchanged, 20,004 `no_ops_row`, 237 `unmapped`.~~ **Actual: 930 corrected, 31,032 unchanged, 19,772 `no_ops_row`, 237 `unmapped`.** The estimate counted the 214 date-shifted rows as corrections; they are not — the shift repairs `game_date`, not the value, so they land in `unchanged` (see spec §5.2).
